...
type Requirement = { id: number; name: string; abbreviation: string | null };
...
type Document = {
  ...
  uploaded_by_relation?: User | null;
  created_at?: string;
};
...
export default function PeoplePage() {
  ...
  const [data, setData] = useState<Paginated<Person> | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await apiFetch<Paginated<Person>>(`/api/people${query}`);
        if (!active) return;
        setData(res);
      } catch (e) {
        if (!active) return;
  ...
  async function openDocs(p: Person) {
    setSelected(p);
    setDocsOpen(true);
    setDocs([]);
    setDocsError(null);
    ...
    try {
      const res = await apiFetch<Paginated<Document>>(
        `/api/documents${toQuery({ documentable_type: 'person', documentable_id: p.id, per_page: 100 })}`,
      );
      setDocs(res.data);
    } catch (e) {
  ...
  async function onCreate(e: FormEvent) {
    e.preventDefault();
    if (!newFullName.trim() || creating) return;
    setCreating(true);
    try {
      ...
      setNewIsActive(true);
      setPage(1);
      const res = await apiFetch<Paginated<Person>>(`/api/people${toQuery({ page: 1, per_page: perPage })}`);
      setData(res);
    } catch (e) {
      alert(e instanceof ApiError ? e.message : 'No se pudo crear la persona.');
    } finally {
      setCreating(false);
    }
  }
  ...
  async function uploadDocument(e: FormEvent) {
    ...
    try {
      ...
      fd.append('documentable_type', 'person');
  ...
  return (
    <div className="page">
      <div className="pageHeader">
        <div>
          <div className="pageTitle">Personas</div>
              ...
              {loading ? (
                ...
                data.data.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td>{p.full_name}</td>
                    <td className="tdMono">{p.document_id ?? '—'}</td>
                    <td>{p.email ?? '—'}</td>
                    <td className="tdMono">{p.phone ?? '—'}</td>
                    <td>{p.is_active ? <span className="pill pillOk">Activo</span> : <span className="pill">Inactivo</span>}</td>
                    <td className="tdMono">{p.documents_count}</td>
                    <td>
                      <button className="btnSoft" type="button" onClick={() => openDocs(p)}>
                        Documentos
                      </button>
                    </td>
                  </tr>
      ...
      <Modal title="Nueva persona" open={createOpen} onClose={() => setCreateOpen(false)}>
        <form onSubmit={onCreate} className="formGrid">
          ...
          <div className="formActions">
            ...
            <button className="btnPrimary" type="submit" disabled={creating || !newFullName.trim()}>
      ...
      <Modal title={selected ? `Documentos — ${selected.full_name}` : 'Documentos'} open={docsOpen} onClose={() => setDocsOpen(false)}>
        {!selected ? null : (
            ...
            <div className="docsRight">
              ...
              <div className="docsList">
                {docsLoading ? (
                  ...
                  <div className="tdMuted">No hay documentos para esta persona.</div>
                )}
              </div>
            </div>
          </div>
